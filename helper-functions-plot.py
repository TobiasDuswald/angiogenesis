...
# This script plots the helper functions h and l for the cell cycle model
...
def plot(fn, func_name, x, dt, xx, a, b):
  ...
  if func_name == "h":
    ...
    for cc in a:
      for xxx in xx:
        plt.plot(x, f(x, cc, xxx, dt), 
                 label=r"$c={}, ".format(cc) + r"\bar{x}" + r"={}$".format(xxx))
